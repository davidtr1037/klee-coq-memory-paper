...
\[ \moReadFunc : \MemoryObjects \times \Vals^{\pointertype} \times \Types \to \Vals \]
The $\moReadFunc$ function receives a memory object $\mo$, a pointer $p$, and a type $\tau$.\footnote{
Here, we assume that $\tau$ is one of the following types: $\bvsort{8}$, $\bvsort{16}$, $\bvsort{32}$, $\bvsort{64}$.
}
...
\[ \moWriteFunc : \MemoryObjects \times \Vals^{\pointertype} \times \Vals \to \MemoryObjects \]
The $\moWriteFunc$ function receives a memory object $\mo$ (\ie $\mobj{\addr}{\size}{\arrayvalue}$), a pointer $p$, and a value $v$.
...
Then, it uses these bytes to return a new memory object $\mobj{\addr}{\size}{\arrayvalue'}$,
...
%When expressions do not adhere to the expected types, \ie they are \emph{ill-typed}, the semantics is undefined (marked by $\bot$).
...
we call the $\allocate$ function defined in~\Cref{sec:memory-modeling}.
...
If that is the case, we remove that memory object from $\memory$ using the $\deallocate$ function.
Otherwise, the result is undefined behavior.
...
For a read operation,
we call the $\moReadFunc$ function (\Cref{sec:memory-modeling}) to read the value from $\mo$ and update the local store accordingly.
%For a write operation,
%we serialize the value $v$ into a sequence of bytes and update the array $\arrayvalue$ accordingly.
For a write operation,
we call the $\moWriteFunc$ function (\Cref{sec:memory-modeling}) to update the contents of $\mo$ accordingly.
