...
\Function{compute-offset}{$\tau, o, \overline{j}$}
    ...
    \If {$\tau = \arraytype{\tau'}{n}$}
        \State $o' \gets \signext{\indextype}{j_1} \cdot \valoftype{|\tau'|}{\indextype}$ \label{line:gep:array-index}
...
\Function{eval-gep}{$\tau, p, \overline{j}$}
    ...
    \State $o \gets \signext{\indextype}{j_0} \cdot \valoftype{|\tau|}{\indextype}$ \label{line:gep:first-index}
    \State $r \gets \Call{compute-offset}{\tau, o, \overline{j'}}$
    \State \Return $p + \zeroext{\pointertype}{r}$ \label{line:gep:add-offset}
\EndFunction
