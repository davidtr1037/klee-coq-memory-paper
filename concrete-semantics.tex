...
First, modeling such memory allocators requires developing a more complex theory of memory (\Cref{sec:concrete-semantics,sec:symbolic-semantics,sec:program-safety,sec:proof-generation}).
...
To simplify the notations, the \llvm module, denoted by $\module$, is implicitly global.
\end{definition}
...
we model this behavior by non-deterministically assigning the variable $x$ to an arbitrary concrete value $\bvval{n}{32}$.
...
The first index is used to move across the elements corresponding to the base pointer type,
...
then the index is multiplied by the type size of the element type (\cref{line:gep:array-index}).
...
\begin{enumerate}
    ...
    \item $\instrat{\location} = \textup{$\codequote{\tcall~\intrinsicfunc{free}(\pointertypeof{\bvsort{8}} \ e)}$}$, \textup{$\evalexprty{e}{\localstore}{\pointertypeof{\bvsort{8}}} = p$},
          and there is no $\mo \in \memory$ such that $\isbaseaddr{p}{\mo}$.
    \item $\instrat{\location} = \textup{$\codequote{x \ \texttt{=} \ \tload^{\tau} \ e}$}$, \textup{$\evalexprty{e}{\localstore}{\pointertypeof{\tau}} = p$},
          and there is no $\mo \in \memory$ such that $\isinbounds{p}{|\tau|}{\mo}$.
...
if for every $c$ such that $c_{0} \multistep c$, $c$ is not an error state (the relation $\multistep$ is the reflexive-transitive closure of $\step$).
\end{definition}
