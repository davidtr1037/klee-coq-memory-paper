...
memory objects, the heap, and the memory allocator.
...
The algorithm uses the function $\convertfunc{\indexwidth}$ which receives a bit-vector $\bvval{n}{w}$ (of type $\bvsort{w}$) and returns a bit-vector of type $\indextype$ as follows:
If the width of $\bvval{n}{w}$ is less than $\indexwidth$ it performs sign-extension,
if the width of $\bvval{n}{w}$ is greater than $\indexwidth$ it performs truncation,
and otherwise it just returns $\bvval{n}{w}$.
%
...
\Function{compute-offset}{$\tau, o, \overline{j}$}
    ...
    \If {$\tau = \arraytype{\tau'}{n}$}
        \State $o' \gets \convert{\indextype}{j_1} \cdot \valoftype{|\tau'|}{\indextype}$ \label{line:gep:array-index}
...
\Function{eval-gep}{$\tau, p, \overline{j}$}
    ...
    \State $o \gets \convert{\indextype}{j_0} \cdot \valoftype{|\tau|}{\indextype}$ \label{line:gep:first-index}
    \State $r \gets \Call{compute-offset}{\tau, o, \overline{j'}}$
    \State \Return $p + \zeroext_{\pointertype}(r)$ \label{line:gep:add-offset}
\EndFunction
