...
%Modern symbolic execution (\SE) engines are mainly designed for identifying bugs in programs, focusing on applicability and performance.
Symbolic execution (\SE) is a program analysis technique with many applications.
Beyond its widespread use for bug finding,
\SE can also be used for program verification if the analysis is \emph{exhaustive}, \ie when all the execution paths are explored.
However, modern \SE engines provide no formal guarantees about the correctness of their analysis results,
so relying on such unverified tools is insufficient for verification tasks.
%In particular,
%for tasks such as program verification, relying on such unverified \SE engines is insufficient.
%One approach is to derive a verified \SE engine using extraction from proof assistants,
%but this was applied only in rather simplistic settings.
One approach to address this challenge is \emph{proof logging},
which works by instrumenting the \SE engine to generate a safety proof that is specific to the analyzed program.
...
We developed a \coq framework for a subset of \llvm IR with memory operations,
and extended \klee to generate safety proofs based in this framework.
% TODO: approach?
We evaluated our approach on APIs from real-world benchmarks,
and found previously unknown issues in the memory modeling in \klee.
