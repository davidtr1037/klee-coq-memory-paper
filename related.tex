...
L\"{o}\"{o}w et al.~\cite{cse-oopsla25} mechanized in \coq a memory-parametric framework for a \textit{while}-like IR language,
and instantiated it on several memory models.
Based in this framework,
they built a prototype \SE engine that provides soundness and completeness guarantees.
However,
their prototype \SE engine comes with limitations (\eg, no support for recursion),
and it was not evaluated on any real-world programming language.
To obtain theoretical results regarding \llvm IR using their framework,
one would need to modify the underlying IR language (and core theory), and mechanize the memory model of \llvm IR,
which is expected to require a significant effort.
There exist more realistic \SE engines~\cite{gillian} (for C and Java) which are based on a similar theory,
but they are not verified and do not produce mechanized proofs.
