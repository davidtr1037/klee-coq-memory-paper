...
Trabish et al.~\cite{klee-rocq} propose a proof-logging approach for a subset of \llvm with integers.
Lundberg et al.~\cite{lundberg2024proof} produce functional correctness proofs for P4 (a DSL for network devices) programs using HOL4.
...
Keuchel et al.~\cite{icfp22:keuchel} built Katamaran, a prototype \SE engine for verifying ISA specifications in a subset of SAIL~\cite{sail} based on separation logic.
However, Katamaran is only semi-automated:
it relies on an interactive proof mode,
requires user-provided lemmas,
and does not support proof automation for bit-vectors.
