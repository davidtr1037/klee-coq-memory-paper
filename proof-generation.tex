...
where \textup{$\instrat{\location} = \codequote{x~\texttt{=}~\tcall^{\pointertype}~\intrinsicfunc{malloc}(e)}$}.
If there exist $n$, $\mo$, and $t$ such that:
\begin{itemize}
    \item $\exists n. \ \evalexprty{e}{\symlocalstore}{\bvsort{64}} \equiv \bvval{n}{64} \land n < max\_signed_{\bvsort{32}}$
    \item $\mo \equiv (\nfa{\memory}, \bvval{n}{32}, \arrayconst{\arrayinitvalue})$
    \item $t.\attrstate = \symstate{\nextinstr{\location}}{\symlocalstore[x \mapsto \addr]}{\symstack}{\memory \mathbin{++} [\mo]}{\pc}$
    \item $\safeet(t)$
\end{itemize}
then $\safeet(\tree(s, [t]))$.
\end{lemma}
...
there might be several derived symbolic states and each such symbolic state should have the following form:
\[
\symstate
    {...}
    {\{ ..., \nameid{arrayidx} \mapsto \bvconst{64}{\addr} \}}
