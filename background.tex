...
which can be either values of type $\Vals^{\bvsort{8}}$ (\ie a byte) or one of the special values $\tundef$ or $\tpoison$.
A heap $\memory \in \Heaps$ is defined as a set of memory objects.
\end{definition}

We assume an implicit \emph{allocator},
which is a stateful function $\Heaps \to \MemoryObjects$,
...
In this paper,
we assume a linear allocator that allocates memory objects consecutively, while maintaining a fixed gap between them,
...
The semantics presented in this paper considers only heap allocations.
...
\Cref{appendix:concrete-semantics} provides the pseudo-code of the semantics, defined as $\evalgep$.
}
