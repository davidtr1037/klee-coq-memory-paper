...
In the sequel,
the bit-widths of the pointer type and the index type are denoted by $\textsf{ptr}$ and $\textsf{idx}$, respectively.
...
\begin{definition}
A \emph{concrete heap}, $\memory ::= \langle \{mo, \ldots\}, \textit{alloc}\rangle$,
according to the memory-object model,
consists of a set of memory objects (defined below, \Cref{def:memory}) and a \emph{memory allocator}, which is a
stateful function $(\textrm{size}: \Nat) \mapsto \langle p, \memory'\rangle$,
yielding a fresh-allocated address $p$ and a new heap state $\memory'$.

We use $\Heaps$ to denote the set of all possible heaps.
\si{do we need this?}
\end{definition}
...
\[ \mo ::= \langle\addr, \size, \arrayvalue\rangle \in \MemoryObjects \quad \quad \MemoryObjects \triangleq \Vals^{\pointertype} \times \Vals^{\indextype} \times (\Z \rightarrow \Vals^{\bvsort{8}} \cup \{\tundef, \tpoison\}) \]
...
In addition, an address interval must be fully contained within
the address domain $\Vals^{\pointertype}$ (\ie $b+s$ must not overflow).
...
and $\memory \in (\MemoryObjects)^{*}$ is a sequence of memory objects.\si{should be $\memory\in\Heaps$ as it also contains the allocator monad?}
\end{definition}
...
we first check if there exists a memory object $\langle\addr, \size, \arrayvalue\rangle$ in the memory $\memory$ such that $p = \addr$.
...
we need to make sure that there exists a memory object $\langle\addr, \size, \arrayvalue\range$ in the memory $\memory$ such that
...
we need to make sure that there exists a memory object $\langle\addr, \size, \arrayvalue\rangle$ in the memory $\memory$ such that
...
We then define when a program---an \llvm module---is considered  safe.
...
\begin{enumerate}
    ...
    \item $\instrat{\location} = \textup{$\codequote{\tcall~\intrinsicfunc{free}(\pointertypeof{\bvsort{8}} \ e)}$}$, \textup{$\evalexprty{e}{\localstore}{\pointertypeof{\bvsort{8}}} = p$}, and $\invalidbaseaddress(\memory, p)$.
    \item $\instrat{\location} = \textup{$\codequote{x \ \texttt{=} \ \tload^{\tau} \ e}$}$, \textup{$\evalexprty{e}{\localstore}{\pointertypeof{\tau}} = p$}, and $\invalidpointer(\memory, p, |\tau|)$.
    \item $\instrat{\location} = \textup{$\codequote{\tstore^{\tau} \ e_1 \ e_2}$}$, \textup{$\evalexprty{e_2}{\localstore}{\pointertypeof{\tau}} = p$}, and $\invalidpointer(\memory, p, |\tau|)$.
