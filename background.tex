...
A simplified version of the $\tgep$ semantics is given in~\Cref{alg:gep}.
...
The $\tgep$ instruction may have flags (\eg \opcode{nuw}, \opcode{nusw}, and \opcode{inbounds}) that impose additional rules.
For example, if the \opcode{nuw} flag is enabled, then the following must hold:
\begin{enumerate}
\item If the type of an index is larger than the index type, the truncation to the index type must preserve the unsigned value.
\item The multiplication of an index by the type size does not wrap the index type in an unsigned sense.
\item The successive addition of each offset (without adding the base address) does not wrap the index type in an unsigned sense.
\end{enumerate}
