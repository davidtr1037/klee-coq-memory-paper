...
% \si{this footnote seems pesky here. the syntax & semantics are
%    minimal throughout. we can mention when discussing the malloc rule that similar principles apply}
%\footnote{
...
\moRead{\mo}{p}{\tau}   & \mbox{read a value of type $\tau$ from memory object $\mo$ starting from byte address $p$} \\
\moWrite{\mo}{p}{\tau}{v} & \mbox{write value $v$ of type $\tau$ into memory object $\mo$ at byte address $p$} \\
\heapRpl{\memory}{\mo}{\mo'} & \mbox{replace memory object $\mo$ with $mo'$ ($\scriptstyle \memory\setminus\{\mo\}\cup\{\mo'\}$)}
\end{array}
$}
\end{array}
\]
...
\begin{definition}
\label{def:memory}
A \emph{concrete heap}, $\memory ::= \{\mo_1, \ldots, \mo_n\}$
consists of a set of \emph{memory objects};
%
...
Each memory object is associated with an address interval,
$[b, b+s)$.
We use $(\MemoryObjects)^*$ to refer to the set of possible heaps.
\end{definition}

We assume the existence of an implicit \emph{allocator},
which is a stateful function $(\MemoryObjects)^* \to \MemoryObjects$, providing a freshly-allocated memory
object whose address interval must not overlap those
of existing memory objects.
