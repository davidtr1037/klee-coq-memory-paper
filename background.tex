...
\Function{eval-gep}{$t, p, i, is$}
    \If {$\tpoison \in \{i\} \cup is$}
        \State \Return $\tpoison$
    \EndIf
    \If {$\tundef \in \{i\} \cup is$}
        \State \Return $\tundef$
    \EndIf
    \State \Return \Call{compute-pointer}{$t, p, i is$}
\EndFunction

\Function{compute-pointer}{$t, p, i, is$}
    \State $o \gets \mathit{trunc}_{\indextype}(i) \cdot |t|$
    \State $r \gets \Call{compute-offset}{t, o, is}$
...
\Function{compute-offset}{$t, i, is$}
    \If {|is| = 0}
        \State \Return $i$
    \EndIf
    \State $is \gets i' :: is'$
    \If {$t = \arraytype{t'}$}
        \State $o \gets i + \mathit{sign\_ext}_{\indextype}(i') \cdot |t'|$
        \State \Return \Call{compute-offset}{$t', t_{add}, is'$}
    \EndIf
    \If {$t = \structtype{ts}$}
        \State $n \gets \mathit{to\_nat}(i')$
        \State $t_{n} \gets \mathit{field\_type}(ts, n)$
        \State $o \gets i + \mathit{field\_offset}(ts, n)$
        \State \Return \Call{compute-offset}{$t_{n}, o, is'$}
    \EndIf
    \State ...
\EndFunction
