...
In \llvm IR, the \emph{data layout} specifies how data is to be laid out in memory.
...
The pointer type specifies the type used to represent pointer values.
...
\begin{algorithm}[t]
\caption{The simplified semantics of $\tgep$ (without attributes).}
\label{alg:gep}
\begin{algorithmic}[1]
...
\Function{compute-offset}{$\tau, o, \overline{j}$}
    ...
    \If {$\tau = \structtype{\overline{\tau}}$}
        \State $n \gets \convert_{\indextype}(j_1)$
        \State $\tau_{n} \gets \mathit{field\_type}(\overline{\tau}, n)$
        \State $o \gets o + \mathit{field\_offset}(\overline{\tau}, n)$ \label{line:struct-index}
...
\begin{enumerate}
    \item $\instrat{\location} = \codequote{\tunreachable}$
    \item $\instrat{\location} = \codequote{x \ \texttt{=} \ op \ \bvsort{w} \ e_1 \ e_2}$, and one of the following holds, where $v_k \triangleq \evalexprty{e_k}{\localstore}{\bvsort{w}}$:
    \begin{enumerate}[left=-3pt]
        \item $op \in \{\opcode{udiv}, \opcode{sdiv}, \opcode{urem}, \opcode{srem}\}$ and $v_2 = \bvval{0}{w}$
        \item $op = \opcode{sdiv}$, $v_1 = \bvval{(-2^{w - 1})}{w}$, and $v_2 = \bvval{(-1)}{w}$
        \item $op \in \{\opcode{shl}, \opcode{lshr}, \opcode{ashr}\}$ and $v_2 = \bvval{n}{w}$, and $n \geq w$
    \end{enumerate}
    \item $\instrat{\location} = \codequote{x \ \texttt{=} \ \tcall^{\pointertype} \ \intrinsicfunc{malloc}(e)}$, and the following holds:
    \begin{enumerate}
        \item $\evalexprty{e}{\sigma}{\tau} > max\_signed_{\bvsort{32}}$
    \end{enumerate}
    \item $\instrat{\location} = \codequote{\tcall~\intrinsicfunc{free}(e)}$, and the following holds:
    \begin{enumerate}
        \item $\evalexprty{e}{\sigma}{\tau} = p$
        \item $\invalidbaseaddress(\memory, p)$
    \end{enumerate}
    \item $\instrat{\location} = \codequote{x \ \texttt{=} \ \tload^{\tau} \ e}$, and the following holds:
    \begin{enumerate}
        \item $\evalexprty{e}{\sigma}{\tau} = p$
        \item $\invalidpointer(\memory, p, |\tau|)$
    \end{enumerate}
    \item $\instrat{\location} = \codequote{\tstore^{\tau} \ e_1 \ e_2}$, and the following holds:
    \begin{enumerate}
        \item $\evalexprty{e_2}{\sigma}{\tau} = p$
        \item $\invalidpointer(\memory, p, |\tau|)$
    \end{enumerate}
    \item $\instrat{\location} = \codequote{x \ \texttt{=} \ \tgep~\pointertypeof{\tau} \ e \ \overline{(\tau_i, e_i)}}$
    \begin{enumerate}
        \item $...$
    \end{enumerate}
\end{enumerate}
