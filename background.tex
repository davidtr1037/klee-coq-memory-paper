...
\begin{definition}
A \emph{concrete heap}, $\memory ::= \heaprepr{ \{mo, \ldots\} }{ \textit{alloc} }$,
...
stateful function $(\textrm{size}: \Nat) \mapsto \heaprepr{ p }{ \memory' }$,
yielding a freshly-allocated address $p$ and a new heap state $\memory'$.
...
\[ \mo ::= \mobj{\addr}{\size}{\arrayvalue} \in \MemoryObjects \quad \quad \MemoryObjects \triangleq \Vals^{\pointertype} \times \Vals^{\indextype} \times (\Z \rightarrow \Vals^{\bvsort{8}} \cup \{\tundef, \tpoison\}) \]
...
Whenever an allocator creates a new memory object,
the address interval of that memory object must not overlap with the address intervals of existing memory objects.
In addition, an address interval must be fully contained within the address sapce, \ie $\addr + \size$ must not overflow.
...
The rules for other instructions (assignments, branches, \etc) were discussed in detail in~\cite{klee-rocq}.
