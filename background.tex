...
In this section,
we provide background on \llvm IR~\cite{llvm},
an intermediate representation used in \llvm,
and on satisfiability modulo theories (SMT).
...
% TODO: the width of the pointer itself is fixed
% TODO: mention the pointer index type?
...
We use the notation $\bvval{n}{w}$ to denote an integer value of type $\bvsort{$w$}$ whose (signed or unsigned) numerical value is $n$.
...
such as \textsf{alloca}, a heap allocation call (\eg \textit{malloc}), or a global variable definition.\footnote{
The semantics presented in the paper support only heap allocations.
However, our implementation supports the other memory-allocation operations as well.
}
...
In the sequel,
the pointer type and index type are denoted by $\pointertype$ and $\indextype$, respectively.

\si{add spec of pointer values from llvmref}
...
\\[1em]
\ruledefsmall{Malloc}{
  \instrat{\location} = \codequote{x~\texttt{=}~\tcall^{\pointertype}~\intrinsicfunc{malloc}(\bvsort{64} ~ e)} \quad
...
%\begin{definition}
%A \emph{concrete heap}, according to the memory-object model,
%is a mapping from (half-open, integer) intervals to arrays of bytes of a corresponding size.
%\[
%\begin{array}{l}
...
%\]
%
%The notation $E[\,]$ denotes a finite, fixed-size arrays of elements of $E$.
%
...
\[ (\addr, \size, \arrayvalue) \in \MemoryObjects \quad \quad \MemoryObjects \triangleq \Vals^{\pointertype} \times \Vals^{\indextype} \times (\Z \rightarrow \Vals^{\bvsort{8}} \cup \{\tundef, \tpoison\}) \]
...
The memory is defined using a heap and an allocator.
...
In addition, an address interval must not wrap around the address space.
% TODO: add this?
% \[ \addr \leq \addr + \size \]
\end{definition}

In this paper,
we assume a linear allocator that allocates memory objects in an ascending order while separating them with a fixed gap,
commonly referred to as a \emph{red-zone}.
Modeling a generic allocator that allows arbitrary locations for memory objects is challenging.
First, modeling such allocators requires developing a more complex theory (\Cref{sec:core}).
...
where $\location \in \Locs$ is a location,
...
we first check if there exists a memory object $(\addr, \size, \arrayvalue)$ in the memory $\memory$ such that $p = \addr$.
...
we need to make sure that there exists a memory object $(\addr, \size, \arrayvalue)$ in the memory $\memory$ such that
...
The indexed type of the first index is the base pointer type,
the indexed type of the second index is the element type of the base pointer type, \etc
...
If the indexed type is a struct type\footnote{The pseudo-code for this case is not shown due to space limitations.},
...
In the following,
we define when an concrete state is considered to be an error state.
Afterwards,
we define when a program, \ie an \llvm module, is considered as safe.
...
\begin{enumerate}
    ...
    \end{enumerate}
    \item $\instrat{\location} = \textup{$\codequote{x \ \texttt{=} \ \tcall^{\pointertype} \ \intrinsicfunc{malloc}(e)}$}$, and \textup{$\evalexprty{e}{\localstore}{\bvsort{64}} > max\_signed_{\indextype}$}.
    \item $\instrat{\location} = \textup{$\codequote{\tcall~\intrinsicfunc{free}(e)}$}$, \textup{$\evalexprty{e}{\localstore}{\pointertype} = p$}, and $\invalidbaseaddress(\memory, p)$.
    \item $\instrat{\location} = \textup{$\codequote{x \ \texttt{=} \ \tload^{\tau} \ e}$}$, \textup{$\evalexprty{e}{\localstore}{\pointertype} = p$}, and $\invalidpointer(\memory, p, |\tau|)$.
    \item $\instrat{\location} = \textup{$\codequote{\tstore^{\tau} \ e_1 \ e_2}$}$, \textup{$\evalexprty{e_2}{\localstore}{\pointertype} = p$}, and $\invalidpointer(\memory, p, |\tau|)$.
    % TODO: $|\overline{(\tau_i, e_i)}| > 0$
    \item $\instrat{\location} = \textup{$\codequote{x \ \texttt{=} \ \tgep~\pointertypeof{\tau} \ e \ \overline{(\tau_i, e_i)}}$}$, and:
    \begin{enumerate}
        \item \textup{$\evalexprty{e}{\localstore}{\pointertype} = p$}, $\evalexprty{e_i}{\localstore}{\tau_i} = j_i$, and $\evalgep(\tau, p, \overline{j_i}) = \tpoison$.
    \end{enumerate}
\end{enumerate}
where:
\[
...
% TODO: In the bit-vector theory
The bit-vector theory defines the $\mathsf{BV}[w]$ sort, which is interpreted as the set $\{0{,}1\}^w$,
and it introduces \emph{interpreted} function symbols for operations over values of bit-vector sorts (\eg $\textit{bvadd}$).
Array theory defines the array sort $\arraysort{I}{V}$, which is parametrized by an index sort $I$ and a value sort $V$.\footnote{
...
%$\mathit{select} : \arraysort{I}{V} \times I \rightarrow V$
%$\mathit{store} : \arraysort{I}{V} \times I \times V \rightarrow \arraysort{I}{V}$
%$K : V \rightarrow \arraysort{I}{V}$

A \emph{structure} $m$ must map interpreted function symbols to their accepted functions over $\{0{,}1\}^w$.
...
%In the sequel, we take a slightly nonstandard approach,
%although it is commonplace in implementations of symbolic execution,
%and identify $\Bool$ with $\mathsf{BV}[1]$,
%where $\true$ and $\false$ correspond to $1$ and $0$, respectively.
%This is done for uniformity of presentation only.

\begin{paragraph}{Notations}
We denote by $m[t]$ the interpretation of the term $t$ in a structure $m$.
%In particular, $m[\varphi] \in \{\true, \false\}$ in the case of formulas.
We denote by $t_1\equiv t_2$ the assertion that $m[t_1] = m[t_2]$ for any structure $m$.
\end{paragraph}
