...
The memory is identified by a heap and an allocator.
The heap is a set memory objects,
and the allocator is an algorithm that defines the allocation scheme of memory objects.
...
\begin{enumerate}
    ...
    \item $\instrat{\location} = \textup{$\codequote{x \ \texttt{=} \ \tcall^{\pointertypeof{\bvsort{8}}} \ \intrinsicfunc{malloc}(\bvsort{64} \ e)}$}$,
          and \textup{$\evalexprty{e}{\localstore}{\bvsort{64}} > \valoftype{max\_signed}{\indextype}$}.
