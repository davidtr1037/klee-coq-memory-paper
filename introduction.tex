...
Although \SE can be used for verification tasks,
...
However, proof logging has not yet been demonstrated on real-world programs that require memory modeling.
...
In this work,
we extend the approach in~\cite{klee-rocq} the support memory operations.
We extend the concrete and symbolic semantics to support memory-related instructions.
We extend the theoretical foundation which allows to prove program safety based on properties of execution trees.
We extend \klee to geneate safety proofs based on our formalization.
To address the gap between the formalized symbolic semantics and the heavily-optimized behavior in \klee,
we develop an additional theory consisting of \emph{interface lemmas} that help to interact with the various parts of our formalization.
...
% implemented ...
% bugs in KLEE
\item Mechanized concrete and symbolic semantics for a subset of \llvm with memory.
\item An extension of \klee that generates safety proofs using interface lemmas.
\item We applied our approach on APIs from real-world benchmarks.
      We identified several issues in \klee related to memory modeling.
\end{itemize}
\end{paragraph}
