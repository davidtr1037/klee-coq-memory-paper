...
In addition, \SE tools use various memory optimizations,
which complicates the proof-logging process, which complicates the proof generation process.
For example, \klee~\cite{klee} uses multiple representations for memory objects, uses SMT encoding optimizations, \etc
Therefore, applying the proof-logging approach for \llvm programs with memory is challenging.
