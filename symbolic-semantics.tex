...
\paragraph{Read}
The \emph{read} operation is defined using the following function:\footnote{
As in~\Cref{sec:memory-modeling}, we assume that $\tau$ is one of the built-in integer types (\bvsort{8}, \bvsort{16}, \bvsort{32}, \bvsort{64}), and $w$ is the bit-width of $\tau$.
}
...
\[ \symmowritefunc{\tau} : \SymMemoryObjects \times \pointertype \times ??? \to \SymMemoryObjects \]
