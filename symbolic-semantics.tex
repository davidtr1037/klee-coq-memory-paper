...
A symbolic stack $\symstack$ \emph{represents} a concrete stack $\stack$ via a structure $m$,
...
$\arrayselect{\smtarray}{\bvconst{\indexsort}{k}}$ represents $\arrayvalue(\valoftype{k}{\indextype})$ via $m$ for every $k$ in the range of the index type.
A symbolic memory $\symmemory$ represents a memory $\memory$ via $m$ if the symbolic memory objects of $\symmemory$ represent the memory objects of $\memory$ via $m$, pointwise.
...
if $s \overapproxvia{m} c$ for some structure $m$.
\end{definition}
