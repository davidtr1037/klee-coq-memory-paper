...
%In this section,
%we present our the symbolic semantics presented in~\cite{klee-rocq} to support the memory-related instructions given in~\Cref{figure:llvm-syntax}.
In this section,
...
% TODO: not that similar...
First, we define the major components of our symbolic memory model.
At a high-level, the symbolic memory model is similar to its concrete counterpart (\Cref{sec:memory-modeling}).
The core difference is that the symbolic memory model uses SMT expressions instead of concrete values (\ie bit-vectors).
...
\end{enumerate}
If such a symbolic memory object can be provided, then the result is the tuple $\langle \memory \cup \mobj{\addr}{\size}{\smtarray}, \addr \rangle$.
...
\[ \symmoreadfunc : \SymMemoryObjects \times \smtsort{\pointerwidth} \times \Types \to ??? \]
The $\symmoreadfunc$ function receives a memory object $\symmo$, a pointer $p$, and a type $\tau$.
...
\[ \symmowritefunc : \SymMemoryObjects \times \pointertype \times ??? \to \SymMemoryObjects \]
The $\symmowritefunc$ function receives a memory object $\symmo$ (\ie $\mobj{\addr}{\size}{\smtarray}$), a pointer $p$, and an SMT expression $e$.
First, it serializes $e$ into a sequence of symbolic bytes.
Then, it uses these symbolic bytes to create a new symbolic memory object $\mobj{\addr}{\size}{\arrayvalue'}$,
...
The symbolic stack $\symstack$ is modified accordingly to hold symbolic stores instead of concrete ones.
...
}
{
  \symstate{\location}{\symlocalstore}{\symstack}{\symmemory}{\pc}
  \symstep
  \symstate{\nextinstr{\location}}{\symlocalstore[x \mapsto \symmoread{\symmo}{p}{\tau}]}{\symstack}{\symmemory}{\pc \land \rangecond(p, |\tau|, \symmo)}
}
\\[1em]
\ruledefsmall{Store}{
