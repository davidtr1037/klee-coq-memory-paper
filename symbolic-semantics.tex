...
In some of the rules, we use the following definitions (where $\symmo \triangleq \mobj{\addr}{\size}{\smtarray}$):
\[ \baseaddrcond(p, n, \symmo) \triangleq p = a, \ \rangecond(p, n, \symmo) \triangleq p < p + n \land p \geq \addr \land p + n < \addr + \size \]
Informally,
...
\ruledefsmall{Malloc}{
  ...
  \allocate(\symmemory, \bvextract_{\indexwidth}(n)) = (\symmemory', p) \quad
}
{
  \symstate{\location}{\symlocalstore}{\symstack}{\symmemory}{\pc}
  \symstep
