...
Let $s \triangleq \symstate{\location}{\symlocalstore}{\symstack}{\symmemory}{\pc}$ be a symbolic state.
Then $s$ is an error state, denoted by $\errorsymstate(c)$, if one of the following holds:
\begin{enumerate}[left=0pt]
    ...
    \end{enumerate}
    % TODO: use bvconst?
    \item $\instrat{\location} = \textup{$\codequote{x \ \texttt{=} \ \tcall^{\pointertype} \ \intrinsicfunc{malloc}(e)}$}$, \textup{$\sat(\pc \land (t > max\_signed_{\bvsort{32}}))$} ($t \triangleq \evalexprty{e}{\symlocalstore}{\tau}$).
