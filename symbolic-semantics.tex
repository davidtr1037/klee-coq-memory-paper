...
\begin{enumerate}[left=0pt]
    ...
    \begin{enumerate}
        ...
        \item $\syminvalidbaseaddress(\symmemory, \pc, p)$
    \end{enumerate}
    \item $\instrat{\location} = \textup{$\codequote{x \ \texttt{=} \ \tload^{\tau} \ e}$}$, and the following holds:
    \begin{enumerate}
        \item $\evalexprty{e}{\symlocalstore}{\tau} = p$
        \item $\syminvalidpointer(\symmemory, \pc, p, |\tau|)$
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \item there exists a model $m$ such that $m \models \varphi \land \overflowcond(\tau, \overline{k_i})$ ($k_i \triangleq \evalexprty{e_i}{\localstore}{\tau_i}$)
    \end{enumerate}
\end{enumerate}
