...
Then, it translates them to a \textit{concat} expression of the appropriate sort.
...
First, it serializes the $e$ into a sequence of symbolic bytes.
Then, it uses these symbolic bytes to return a new symbolic memory object $\mobj{\addr}{\size}{\arrayvalue'}$,
where $\smtarray'$ is derived from $\smyarray$ using store expressions at the corresponding offsets.
