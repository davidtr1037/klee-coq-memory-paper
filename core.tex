\section{From Symbolic Execution to Sound Safety Proofs}
\label{sec:core}

\dt{What is sound in this context?}
...
\dt{add the formal def?}
In order to compose the proof, these two claims need to be established:
\begin{enumerate}
\item The symbolic states encountered during the symbolic execution \emph{represent only safe (concrete) states}.
\item \emph{All reachable (concrete) states} are
represented by at least one such symbolic state.
\end{enumerate}

\dt{define represents formally?}
By ``represents'', we intuitively mean that a concrete state $\sigma$ \dt{$\sigma$ is for stores} is equal to an instantiation of a symbolic state $\tilde\sigma$
by assigning some concrete values to the symbolic
variables of $\tilde\sigma$ and evaluating the resulting expressions.
(The assigned values must respect $\tilde\sigma$'s \emph{path constraint}.)

\dt{the first part also relies on symbolic semantics}
The first claim is relatively easy to ascertain,
using the checks and queries that the symbolic
execution itself performs.
For the second claim, \cite{cpp2025} offers a
two-tiered approach by introducing a \emph{symbolic
semantics} as an intermediary between the symbolic
states and the concrete semantics \dt{states?}.
The purpose of this layering is to defuse the
inherent high complexity that is forever involved
when reasoning about the set of reachable states \dt{concrete or symbolic?}.
...
The development described in this section consists
of two main parts:
\begin{enumerate}
\item Defining the symbolic semantics of memory-%
access instructions and of expressions involving
pointer data types; and, importantly, establishing
the \emph{reachability completeness} property,
showing that the new semantics is true to the
aformentioned concrete semantics.
\item A proof-generation procedure embedded in a
symbolic execution engine (KLEE), which emits Coq \dt{Rocq}
definitions mirrorring the symbolic execution tree,
along with \emph{proofs that the execution tree is
safe}---that is, satisfies the two claims above\si{reference more clearly which ones}---%
using the reachability completeness property and
a few supporting theorems.
\end{enumerate}
