\section{Proof Logging for Memory-Manipulating Programs}
\label{sec:core}

The proof-logging approach presented in~\cite{klee-rocq} is restricted to a subset of \llvm with integers.
We now outline the additional effort required to extend that approach to a subset of \llvm with memory operations.

We extend the semantics to incorporate a memory model of \llvm and to support memory-related instructions.
Then, we extend the proof of the theoretical foundation accordingly.
Indeed, extending the formalization accordingly proved to be arduous:
The original \coq formalization in~\cite{klee-rocq} consists of roughly 18,000 lines,
whereas our extended formalization consists of roughly 40,000 lines,
representing a substantial additional effort.

% proof generation
% TODO: we also need to check that it is not an error state
To prove that an execution tree is safe,
we need to prove that if a symbolic state derived by the symbolic semantics is feasible,
\ie its path constraints are satisfiable,
then it can be paired with an equivalent symbolic state in the execution tree.
In practice, the \SE engine performs various optimizations,
and as a result the symbolic states computed during the analysis,
\ie the ones in the execution tree,
may differ from those derived by the symbolic semantics.
To facilitate proof automation,
we introduce interface lemmas that help bridge this gap.

In~\Cref{sec:symbolic-semantics},
we present our extended symbolic semantics.
In~\Cref{sec:program-safety},
we formalize the main theorem of our theoretical foundation.
In~\Cref{sec:proof-generation},
we present the extended instrumentation of the \SE engine.

%The framework introduced in~\cite{klee-rocq} defines
...
%\cite{klee-rocq} proposed a two-tier approach,
...
\textbf{\textit{Module Assumptions.}}
