...
\paragraph{\textbf{\textit{Incorrect Interpretation of $\tgep$.}}}
We found that the interpretation of $\tgep$ in \klee is not fully compliant with the \llvm reference,
as it does not account for overflows when computing the offset.
We modified the interpretation of $\tgep$ as follows:
First, we modified the offset computation to be in accordance with the index type $\bvsort{32}$,
to make it consistent with the index sort $\smtsort{32}$ of the SMT arrays.
