...
\Function{compute-offset}{$\tau, o, \overline{j}$}
    ...
    \If {$\tau = \arraytype{\tau'}$}
        \State $o \gets \bvadd_{64}(o, \bvmul_{64}(\bvsext_{64}(j_1), \bvconst_{64}(|\tau'|)))$ \label{line:array-index}
...
\Function{compute-pointer}{$\tau, p, j_0, \overline{j}$}
    \State $o \gets \bvmul_{64}(\bvsext_{64}(j_0), \bvconst_{64}(|\tau|))$ \label{line:first-index}
